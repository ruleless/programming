...
int main()
{
	...
	int t,fc,gc,hc,adc,i,j,adc1,tempc1,tempc;
	int*f,*g,*h,*ad,*ad1,*temp1,*temp;
	scanf("%d",&t);
	for(i=1;i<=t;i++)
	{
		...
		while(adc>=hc)
		{
			...
			for(j=1;j<tempc;j++)
				temp[j]=0;
			mul(temp1,tempc1,temp,tempc,h,hc);
			//for(j=0;j<tempc1;j++)
			//{
			//	printf("%d ",temp1[j]);
			//}
			sub(ad,adc,ad1,adc1,temp1,tempc1);
		}
		printf("%d ",adc);
		for(j=0;j<adc;j++)
			printf("%d ",ad[j]);
		putchar('\n');
		delete []f;delete[]g;delete[]h;delete[]ad;delete[]ad1;delete[]temp1;delete[]temp;
	}
	return 0;
}