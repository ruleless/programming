...
bool work()
{
	...
	for(i=0;i<n;i++)
	{
		for(j=i+1;j<n;j++)
		{
			if( !(seg[i].A.x==seg[j].A.x&&seg[i].A.y==seg[j].A.y) )
			{
				if( f(seg[i].A,seg[j].A,i,j) )
					return true;
			}
			if( !(seg[i].A.x==seg[j].B.x&&seg[i].A.y==seg[j].B.y) )
			{
				if( f(seg[i].A,seg[j].B,i,j) )
					return true;
			}
			if( !(seg[i].B.x==seg[j].A.x&&seg[i].B.y==seg[j].A.y) )
			{
				if( f(seg[i].B,seg[j].A,i,j) )
					return true;
			}
			if( !(seg[i].B.x==seg[j].B.x&&seg[i].B.y==seg[j].B.y) )
			{
